    ...
    public class MyLocationListener implements BDLocationListener {             //封装定位结果
        public void onReceiveLocation(BDLocation location) {
            if (location == null || mMapView == null)
            {
                ...
                Toast.makeText(CarActivity.this, "Location null", Toast.LENGTH_SHORT).show();
                return ;
            }
...
//            设置定位图标     定位地点为图标中央
            BitmapDescriptor mCurrentMarket = BitmapDescriptorFactory.fromResource(R.drawable.ic_navigation);
...
//                drawAreaInMap(result);
            ...
            if (i < 1) {
                Toast.makeText(CarActivity.this, "附近没有找到出租车", Toast.LENGTH_SHORT).show();
            }
            br.close();
            is.close();
            pw.close();
...
//        LatLng pt3 = new LatLng(latS + drawRow * addLa, lonS + (drawCol - 1) * addLon);
    ...
    private int FindArea() {
        if (mCurrentLatitude < latS || mCurrentLatitude >= latE || mCurrentLongitude < lonS || mCurrentLongitude >= lonE) {
            Toast.makeText(CarActivity.this, "对不起，快速召车暂时只支持湘潭部分地区", Toast.LENGTH_SHORT).show();
            return -1;
        } else {
